...
def calculate_columns(
    ...
    data["Moving Avg (First) STD"] = data["close"].rolling(int(first_mov_avg_res)).std()
...
def mean_reversion_crypto_algo(
    ...
    if curr_order["Has Position"]:
        if (
            ...
            if curr_order["Total Profit"] < 0:
                curr_order["Total Losses"] += curr_order["Total Profit"]
            else:
                curr_order["Total Gains"] += curr_order["Total Profit"]

    return curr_order, investment


@ray.remote  # Assigns a fraction of the GPU per task
def optimization_loop(data, res, fma, sma, der, win):
    try:
        orders_dict = {}
        curr_order = reset_order()
        initial_investment = 10_000
        profit_factor = 0
        time_steps_ = float(np.floor((end_time - start_time) / timedelta(minutes=res)))
...
def run_optimal_solution(params: dict, investment=10_000):
    ...
    for step in range(int(time_steps_)):
        ...
        curr_order, investment = mean_reversion_crypto_algo(
            curr_row=curr_row,
            curr_order=curr_order,
...
def graph_optimal_solution(
    ...
    for _, order in optimized_orders.items():
        try:
            ...
            if order["Total Profit"] >= 0:
                successful_trades += 1
                sum_profit += order["Total Profit"]
                if best_trade < order["Total Profit"]:
                    best_trade = order["Total Profit"]
                ...
                failed_trades += 1
                sum_profit += order["Total Profit"]
                if worst_trade > order["Total Profit"]:
                    worst_trade = order["Total Profit"]
...
def export_optimal_data(best_params, optimized_orders):
    with open(
        Path(
            json_path,
...
def main_ray():
    ...
    data_objects = {res: ray.put(get_crypto_data(res, initial_time, end_time, TICKER_SYMBOL)) for res in resolution_range}
    ...
    for res in resolution_range:
        ...
        combinations = list(
            itertools.product(
                [data_id], [res], first_mov_avg_range, second_mov_avg_range, deriv_cutoff_range, win_length_range
            )
        )

        # Submit tasks in bulk
        futures.extend([optimization_loop.remote(*combo) for combo in combinations])

    # Display progress bar
    with tqdm(total=len(futures), desc="Processing", unit="task") as pbar:
        while futures:
            ready_futures, futures = ray.wait(futures, num_returns=min(10, len(futures)), timeout=1)  # Fetch multiple results
                ...
                for sma in second_mov_avg_range:
                    for der in deriv_cutoff_range:
                        for win in win_length_range:
                            result = optimization_loop(data, res, fma, sma, der, win)
...
if __name__ == "__main__":
    ...
    resolution_range = [30, 40, 50]
    first_mov_avg_range = [0.2, 0.3, 0.5]
    second_mov_avg_range = [5, 6, 7, 9]
    win_length_range = [9, 11, 13]
    deriv_cutoff_range = [0.8, 1.0, 1.2]
