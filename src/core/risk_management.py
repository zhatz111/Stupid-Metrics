...
class RiskManager:
    def __init__(
        ...
        self.max_drawdown = max_drawdown  # e.g. 20% max allowable drawdown
        self.stop_loss_pct = stop_loss_pct
        self.target_price_pct = target_price_pct
        self.leverage = leverage
        self.equity_curve = []  # track PnL over time for drawdown calcs

    def validate_trade(self, current_capital) -> bool:
        """
        ...
        position_size = min(max_capital_risk, current_capital * self.leverage)
        return position_size <= (current_capital * self.leverage), position_size

    def check_drawdown_limit(
        self, equity_curve, initial_capital, current_capital
    ) -> bool:
        """Return False if max drawdown exceeded."""
        peak = max(equity_curve, default=initial_capital)
